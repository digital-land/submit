...
class CheckAnswersController extends PageController {
  ...
  async createJiraServiceRequest (req, res, next) {
    const data = {
      ...
      organisation: JSON.parse(req.sessionModel.get('lpa') ?? '{}'),
      dataset: req.sessionModel.get('dataset'),
      documentationUrl: req.sessionModel.get('documentation-url'),
      endpoint: req.sessionModel.get('endpoint-url')
    }

    const summary = `Dataset URL request: ${data.organisation.name} for ${data.dataset}`
    const description = `A new dataset request has been made by *${data.name}* from *${data.organisation.name} (${data.organisation.id})* for the dataset *${data.dataset}*.\n\n
    ...
    *Details:*\n
    ...
    const csv = [
      ...
      [data.organisation.id, data.dataset, data.documentationUrl, data.endpoint, dateNow, 'ogl3']
    ].map(row => row.join(',')).join('\n')

    const file = new File([csv], `request-${data.organisation.id}-${data.dataset}-${dateNow}.csv`, { type: 'text/csv' })
