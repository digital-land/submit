...
describe('fetchDatasets cache', () => {
  ...
  afterEach(() => { delete config.redis })

  it('works without Redis configured and preserves empty list responses', async () => {
    delete config.redis
    axios.get.mockResolvedValueOnce({ data: { dataset: 'tree' } })
      .mockResolvedValueOnce({ data: { datasets: [] } })
    expect((await api.fetchDatasets({ dataset: 'tree' })).formattedData).toEqual([{ dataset: 'tree' }])
    expect((await api.fetchDatasets({})).formattedData).toEqual([])
